...
# parse arguments
#
# ./switch-to-blue.sh -rd ./temp -rn rollback
#
# ==============================
...
# ==============================
# replace config to blue
# ==============================

record_rollback "sudo nginx -s reload"    # record nginx reload in case for rollback

if ! (echo "set \$deployment blue;" | sudo tee "${CONF_PATH}" >/dev/null); then
  log "Failed to update nginx config" error
  exit 1
fi

# record rollback
record_rollback "echo 'set \$deployment green;' | sudo tee '${CONF_PATH}' >/dev/null"
...
log "Switched to blue successfully" success
